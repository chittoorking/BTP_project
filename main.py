...
if add_selectbox == 'About':
    ...
    st.markdown('We are predicting the final positions and velocities based on positions , velocities and torques and predicting any anomalies that might could happen',unsafe_allow_html=True)
      ...
      P_j1_t = st.number_input("P_j1_t 0.07 to 0.08",step=0.001)
      P_j2_t = st.number_input("P_j2_t -0.05 to -0.04",step=0.001)
      P_j3_t = st.number_input("P_j3_t -0.17 to -0.16",step=0.001)
      P_j4_t = st.number_input("P_j4_t -1.67 to -1.60",step=0.001)
      P_j5_t = st.number_input("P_j5_t 0.1 to 0.11",step=0.001)
      P_j6_t = st.number_input("P_j6_t 0.89 to 0.9",step=0.001)
      P_j7_t = st.number_input("P_j7_t 0.5 to 0.52",step=0.001)
      V_j1_t = st.number_input("V_j1_t -0.0015 to 0.006",step=0.001)
      V_j2_t = st.number_input("V_j2_t -0.002 to 0.02",step=0.001)
      V_j3_t = st.number_input("V_j3_t 0.004 to 0.2",step=0.001)
      V_j4_t = st.number_input("V_j4_t -0.014 to 0.016",step=0.001)
      V_j5_t = st.number_input("V_j5_t -0.008 to 0.017",step=0.001)
      V_j6_t = st.number_input("V_j6_t -0.018 to 0.014",step=0.001)
      V_j7_t = st.number_input("V_j7_t -0.013 to 0.013",step=0.001)
      T_j1_t = st.number_input("T_j1_t 1.24 to 1.8",step=0.001)
      T_j2_t = st.number_input("T_j2_t -0.73 to -0.46",step=0.001)
      T_j3_t = st.number_input("T_j3_t -0.31 to -0.1",step=0.001)
      T_j4_t = st.number_input("T_j4_t -0.28 to -0.1",step=0.001)
      T_j5_t = st.number_input("T_j5_t -0.025 to 0.129",step=0.001)
      T_j6_t = st.number_input("T_j6_t -1.98 to -1.94",step=0.001)
      T_j7_t = st.number_input("T_j7_t -0.3 to 0.0",step=0.001)
