...
if add_selectbox == 'About':
    ...
    st.markdown('We are predicting the final positions and velocities based on positions , velocities and torques and predicting any anomalies that might could happen',unsafe_allow_html=True)
      ...
      P_j1_t = st.number_input("P_j1_t 0.07 to 0.08", min_value=0.05000, max_value=0.10000, step=0.00001)
      P_j2_t = st.number_input("P_j2_t -0.05 to -0.04", min_value=-0.06000, max_value=-0.03000, step=0.00001)
      P_j3_t = st.number_input("P_j3_t -0.17 to -0.16", min_value=-0.20000, max_value=-0.10000, step=0.00001)
      P_j4_t = st.number_input("P_j4_t -1.67 to -1.60", min_value=-1.80000, max_value=-1.40000, step=0.00001)
      P_j5_t = st.number_input("P_j5_t 0.1 to 0.11", min_value=0.05000, max_value=0.20000, step=0.00001)
      P_j6_t = st.number_input("P_j6_t 0.89 to 0.9", min_value=0.89000, max_value=1.00000, step=0.00001)
      P_j7_t = st.number_input("P_j7_t 0.5 to 0.52", min_value=0.40000, max_value=0.60000, step=0.00001)
      V_j1_t = st.number_input("V_j1_t -0.0015 to 0.006", min_value=-0.00150, max_value=0.00600, step=0.00001)
      V_j2_t = st.number_input("V_j2_t -0.002 to 0.02", min_value=-0.00200, max_value=0.02000, step=0.00001)
      V_j3_t = st.number_input("V_j3_t 0.004 to 0.2", min_value=0.00400, max_value=0.20000, step=0.00001)
      V_j4_t = st.number_input("V_j4_t -0.014 to 0.016", min_value=-0.01400, max_value=0.01600, step=0.00001)
      V_j5_t = st.number_input("V_j5_t -0.008 to 0.017", min_value=-0.00800, max_value=0.01700, step=0.00001)
      V_j6_t = st.number_input("V_j6_t -0.018 to 0.014", min_value=-0.01800, max_value=0.01400, step=0.00001)
      V_j7_t = st.number_input("V_j7_t -0.013 to 0.013", min_value=0.00100, max_value=0.01300, step=0.00001)
      T_j1_t = st.number_input("T_j1_t 1.24 to 1.8", min_value=1.24000, max_value=1.80000, step=0.00001)
      T_j2_t = st.number_input("T_j2_t -0.73 to -0.46", min_value=-0.73000, max_value=-0.46000, step=0.00001)
      T_j3_t = st.number_input("T_j3_t -0.31 to -0.1", min_value=-0.31000, max_value=-0.10000, step=0.00001)
      T_j4_t = st.number_input("T_j4_t -0.28 to -0.1", min_value=-0.28000, max_value=-0.10000, step=0.00001)
      T_j5_t = st.number_input("T_j5_t -0.025 to 0.129", min_value=-0.02500, max_value=0.12900, step=0.00001)
      T_j6_t = st.number_input("T_j6_t -1.98 to -1.94", min_value=-1.98000, max_value=-1.94000, step=0.00001)
      T_j7_t = st.number_input("T_j7_t -0.3 to 0.0", min_value=-0.30000, max_value=0.00000, step=0.00001)
      ...
      if submit:
          ...
          st.write('Hi',name)
          st.subheader('The predicted parameters are')
          st.write(prediction)
          anomaly=anomaly_detection.predict(prediction)
          st.write('Anomaly Status',anomaly)
