...
if add_selectbox == 'About':
    ...
    st.markdown('We are predicting the final positions and velocities based on positions , velocities and torques and predicting any anomalies that might could happen',unsafe_allow_html=True)
      ...
      P_j1_t = st.number_input("P_j1_t 0.07 to 0.08", min_value=0.05, max_value=0.1, step=0.00001)
      P_j2_t = st.number_input("P_j2_t -0.05 to -0.04", min_value=-0.06, max_value=-0.03, step=0.00001)
      P_j3_t = st.number_input("P_j3_t -0.17 to -0.16", min_value=-0.2, max_value=-0.1, step=0.00001)
      P_j4_t = st.number_input("P_j4_t -1.67 to -1.60", min_value=-1.8, max_value=-1.4, step=0.00001)
      P_j5_t = st.number_input("P_j5_t 0.1 to 0.11", min_value=0.05, max_value=0.2, step=0.00001)
      P_j6_t = st.number_input("P_j6_t 0.89 to 0.9", min_value=0.89, max_value=1.0, step=0.00001)
      P_j7_t = st.number_input("P_j7_t 0.5 to 0.52", min_value=0.4, max_value=0.6, step=0.00001)
